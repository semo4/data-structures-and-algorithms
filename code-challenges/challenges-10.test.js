...
let result =[];
newarr.forEach(element=> result.push(element.split('').reverse().join('')));
return result;
...
const count = (target, input) => {
  ...
  let newArr = [];
  input.forEach(arr=>{
    newArr.push(arr.filter(element => element === target))
  })
...
const totalSum = (input) => {
  ...
  let sum =0;
  input.map(element =>{
    element.forEach(number => sum =sum +number )
  })
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(element=>{
    arr.push(element.filter(number=> typeof number== 'number'&& number %5===0).map(number=>Math.pow(2,number)))
  })
  return arr;
};
...
let findMaleAndFemale = (data) => {
  ...
  let result = arr.map(element=> element['name']).join(' and ')
  return result;
};
